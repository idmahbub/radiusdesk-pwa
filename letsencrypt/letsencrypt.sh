...
ROUTEROS_PRIVATE_KEY=/root/.ssh/id_rsa
ROUTEROS_PUBLIC_KEY=/root/.ssh/id_rsa.pub
#ROUTEROS_HOST=$(cat routers)

# Certification data
DOMAIN=netlogin.wicsolution.com
...
if [ ! $RESULT == 0 ]; then
  ...
  scp $ROUTEROS_PUBLIC_KEY "$ROUTEROS_USER"@"$ROUTEROS_HOST":"id_rsa.pub"
  if [echo $? == 0 ]; then
...
sleep 1
scp -q -i "$ROUTEROS_PRIVATE_KEY" "$CERTIFICATE" "$ROUTEROS_USER"@"$ROUTEROS_HOST":"$DOMAIN.pem"
sleep 2
...
scp -q -i "$ROUTEROS_PRIVATE_KEY" "$KEY" "$ROUTEROS_USER"@"$ROUTEROS_HOST":"$DOMAIN.key"
sleep 2
